...
| `display_mode` | `mirrored` keeps every display on one wallpaper, cursor and schedule (the default). `independent` preserves connector assignments and targeted rules while the schema-4 runtime support is completed. | `"mirrored"` |
| `theme_source_connector` | Display whose wallpaper drives Noctalia's one shell-wide palette in independent mode. Empty follows the primary display, otherwise the first active display. A detached connector is preserved and temporarily falls back rather than being rewritten. | `""` |
