...
by the Python application. Schema version `2` is intentionally strict. An
...
The packaged unit runs `wall-in-one --write-config` as `ExecStartPre`, then
uses `--wait-for-config` for Rust. This headless compiler reads the same app
settings and authoring stores as the GUI, scans the library, atomically replaces
an outdated document, and exits. It does not import GTK or present a window.
The service remains read-only and still exits on invalid or mismatched bytes;
the wait mode changes startup lifetime only and does not weaken schema checks.
