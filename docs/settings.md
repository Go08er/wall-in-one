...
| `display_mode` | `mirrored` keeps every display on one wallpaper, cursor and schedule (the default). `independent` preserves connector assignments and targeted rules while runtime execution is completed; this staged service build rejects it rather than using global semantics. | `"mirrored"` |
| `theme_source_connector` | Display whose wallpaper drives Noctalia's one shell-wide palette. Independent mode requires one connector token with no whitespace; its UI starts with the first live display. A detached saved connector is preserved and temporarily falls back rather than being rewritten. Mirrored mode may keep it dormant. | `""` |
